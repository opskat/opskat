...
- **NEEDS MFA** (first stderr line, exit code 3): The SSH server requires multi-factor authentication and opsctl had no way to answer it (no code given, no interactive terminal). Nothing ran on the server. Ask the user for the current one-time code (or get it from a TOTP tool you have been given), then retry the same command with `OPSKAT_MFA_CODE=<code>`. Never guess a code. If the retry fails with an MFA verification error, do not retry with the same code — ask for a fresh one.
