...
// testAsset is the exec target the host names for every call below; the tools
// never see an asset id in their arguments.
var testAsset = opskat.Asset{ID: 7, Name: "team runbooks", Type: "notebook"}

func newHost(cfg notebookConfig) *opskat.TestHost {
	return opskat.NewTestHost(opskat.WithAssetConfig(testAsset.ID, cfg))
}
...
func TestNotebookTools(t *testing.T) {
	Convey("notebook tools", t, func() {
		...
		Convey("a stored note is listed and read back", func() {
			_, err := host.CallTool(testAsset, "note_put", putArgs{
			...
			listed, err := host.CallTool(testAsset, "note_list", listArgs{})
			So(err, ShouldBeNil)
			var list struct {
			...
			got, err := host.CallTool(testAsset, "note_get", getArgs{Key: "runbook/failover"})
			So(err, ShouldBeNil)
			var n note
			decode(t, got, &n)
		...
		Convey("writing an existing key overwrites it instead of adding one", func() {
			first, err := host.CallTool(testAsset, "note_put", putArgs{Key: "scratch", Content: "a"})
			So(err, ShouldBeNil)
			var created struct {
			...
			second, err := host.CallTool(testAsset, "note_put", putArgs{Key: "scratch", Content: "b"})
			So(err, ShouldBeNil)
			...
			got, err := host.CallTool(testAsset, "note_get", getArgs{Key: "scratch"})
			So(err, ShouldBeNil)
			var n note
			decode(t, got, &n)
		...
		Convey("listing filters by key prefix", func() {
			for _, key := range []string{"runbook/failover", "runbook/restore", "incident/2026-03-11"} {
				_, err := host.CallTool(testAsset, "note_put", putArgs{Key: key, Content: "x"})
				So(err, ShouldBeNil)
			}

			listed, err := host.CallTool(testAsset, "note_list", listArgs{Prefix: "runbook/"})
			So(err, ShouldBeNil)
			var list struct {
				Count int           `json:"count"`
				Notes []noteSummary `json:"notes"`
		...
		Convey("a deleted note is gone", func() {
			_, err := host.CallTool(testAsset, "note_put", putArgs{Key: "scratch", Content: "x"})
			So(err, ShouldBeNil)

			_, err = host.CallTool(testAsset, "note_delete", deleteArgs{Key: "scratch"})
			So(err, ShouldBeNil)

			_, err = host.CallTool(testAsset, "note_get", getArgs{Key: "scratch"})
		...
		Convey("deleting a note that does not exist is an error, not a silent success", func() {
			_, err := host.CallTool(testAsset, "note_delete", deleteArgs{Key: "nope"})
			So(err, ShouldNotBeNil)
			So(err.Error(), ShouldContainSubstring, `has no note "nope"`)
		})

		Convey("a key the store cannot address is refused", func() {
			_, err := host.CallTool(testAsset, "note_put", putArgs{Key: "../escape", Content: "x"})
			So(err, ShouldNotBeNil)
	...
	Convey("notebooks are separated by the asset's configured name", t, func() {
		otherAsset := opskat.Asset{ID: 8, Name: "other team", Type: "notebook"}
		host := opskat.NewTestHost(
			opskat.WithAssetConfig(testAsset.ID, notebookConfig{Notebook: "team-runbooks"}),
			opskat.WithAssetConfig(otherAsset.ID, notebookConfig{Notebook: "other-team"}),
		)
		defer host.Close()

		_, err := host.CallTool(testAsset, "note_put", putArgs{Key: "shared", Content: "x"})
		So(err, ShouldBeNil)

		listed, err := host.CallTool(otherAsset, "note_list", listArgs{})
		So(err, ShouldBeNil)
		var list struct {
			Notebook string `json:"notebook"`
			Count    int    `json:"count"`
	...
	Convey("the note limit from the asset config is enforced", t, func() {
		...
		_, err := host.CallTool(testAsset, "note_put", putArgs{Key: "first", Content: "x"})
		So(err, ShouldBeNil)

		_, err = host.CallTool(testAsset, "note_put", putArgs{Key: "second", Content: "x"})
		So(err, ShouldNotBeNil)
		So(err.Error(), ShouldContainSubstring, "maxNotes=1")

		Convey("but overwriting an existing note still works at the limit", func() {
			_, err := host.CallTool(testAsset, "note_put", putArgs{Key: "first", Content: "y"})
			So(err, ShouldBeNil)
		})
	})
	...
	Convey("an asset whose config is unusable fails the call instead of writing somewhere else", t, func() {
		...
		_, err := host.CallTool(testAsset, "note_list", listArgs{})
		So(err, ShouldNotBeNil)
