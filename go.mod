...
// etcd v3.5.x server (used only by `-tags integration` test in internal/connpool)
// still calls otelgrpc.UnaryServerInterceptor / StreamServerInterceptor, which
// were removed in otelgrpc v0.61.0. cago-frame pulls v0.65.0; MVS picks that
// for the whole build, breaking the embed.Etcd compile. Pin to v0.60.0 — the
