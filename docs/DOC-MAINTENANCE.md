...
| [`./references/windows-local-ipc.md`](./references/windows-local-ipc.md) | The **local IPC transport** contract behind the two desktop↔`opsctl` endpoints: Windows named pipes vs Unix sockets, endpoint naming/permissions, the both-binaries upgrade requirement, what CI verifies, and the manual AF_UNIX probe. (Transport only — `ARCHITECTURE.md` §8 owns the flows those endpoints carry.) |
