...
**【实施期更正】这条豁免落到代码里是 `if !derived { return policyStrings }`，而它分不清两种来源。** 手写 pattern 与传输适配器给出的 `ApprovalSubject` 都是策略串形状，因此都走 `derived == false` 这条分支。任务 8 的评审证实了后果：`cp ./a.txt s3-prod:/mybucket`（目的地少了 key）产出主体 `object.write mybucket/`，原样落库；而 `object.write mybucket/` 与 `object.write mybucket` **作为规则逐字节等价**（`matchOSSResource` 在 `ruleKey == ""` 时直接 `return true`，`splitOSSResource` 对两种写法都给出空 key，D5 原文也把两者并列）。一次"始终允许"因此换来常驻的整桶写授权；bucket 段带通配还能换来跨桶授权。
...
- OSS 端点剥掉前导 `/` 后必须是 `<bucket>/<key>`：只有桶名（无 `/`）或以 `/` 结尾（是前缀不是对象）一律报错，不猜。
  ...
  **【实施期澄清】这条约束的作用域是"该路径指名一个对象"的场合**，不是所有场合。本节写于递归/通配进入范围之前（那是第四次扩展才加的），与 §6.5 里 `opsctl cp -r 's3-prod:/bucket/logs/' ./logs/` 这个明写支持的例子直接冲突；照最大范围读，§6.5 自己的例子就不可能成立。正确的读法与本地适配器逐条对齐：

  - `OpenRead` / `Write` —— 路径必须指名对象，前缀形状一律报错。
  - `List` —— 前缀形状**在 `recursive` 时接受**，否则报错，与本地适配器 `"%q is a directory; set recursive to transfer it"` 是同一个答案。

  也就是说"报错"针对的是**猜**：把前缀当对象读、或把对象当前缀展开。用户明写 `-r` 时前缀不是歧义，是意图。
...
已知代价：`opsctl cp 3:/a 3:/b dst/` 现在会在任何审批之前对每个指名源做一次远端 `List`（一次 stat），此前那步被 `DirList` 审批挡着。没有任何东西被枚举，但"这个路径在那台资产上存不存在"因此成了无需授权即可回答的问题。判为可接受：调用者已经拥有该资产的 opsctl 访问权，且同样的信息本来也能从随后的传输失败里得到，只是更晚。
...
**【实施期更正 · 用户裁定】批量审批没有"始终允许"，因此多源 cp 不落常驻 grant。** 本节原先写的是"用户点『始终允许』时，`SaveGrantPatternsForApproval` 按条落 grant，重跑同一条 cp 直接命中、不再打断"。落到代码里不成立：`ParseApprovalResponse` 只对 `single` / `local_tool` 放行 `allowAll`（`internal/ai/permission/approval.go:72`），而 grant 只在 `ApprovalAllowAll` 时才落库（`checker.go:275`）；`ApprovalKindBatch` 的注释本身就写着"只允许整批本次放行或拒绝"，前端的「记住」开关也只给那两种 kind（`ApprovalBlock.tsx:58,248`）。多源 cp 走的正是 `batch`，所以"始终允许"在这条路上是一个 parse error，不是一次授权。

要让它成立有两种改法，两种都被否决：让 `batch` 支持 `allowAll` 会顺带给 `batch_exec`（跨资产批量执行 shell 命令）与 opsctl 批量审批开同一个口子，是 issue 之外的、朝放宽方向的行为变更；新开一个 cp 专用的审批 kind 则要同时接后端与两个前端弹框，成本不属于本轮。**因此接受这个缺口：多源 cp 的每一次执行都要重新批准全部条目。** 单源 cp 不受影响（走 `single`，"始终允许"照常可用），`exec` 面的 OSS 命令也不受影响。跟进另开 issue。
...
- **快速失败**：任一 entry 传输出错立即中止，退出码非零，报 `已传输 N/M` 并点名失败的那条。不采用 POSIX `cp -r` 的"继续并最终非零"——每一个已传输的字节都是一次已批准的副作用，出意外后继续会留下一个看起来完整、实际残缺的目的地，而"残缺"这件事从目的端是看不出来的。**【实施期更正】**原文在这里还写着"重跑代价很低：grant 已经落库，第二次不再打断"——按上文的裁定，多源 cp 根本不落 grant，重跑要重新批准全部条目，这句话不成立。快速失败的理由因此只剩前一半，但那一半本来就是承重的那半：中止留下的是一个已知点位的残缺目的地，继续留下的是一个位置未知的残缺目的地。重新批准是这个选择的**代价**，不是它的依据。
...
| **D10 作用域** | 「不记录签名参数」这条**只覆盖审计库**（`audit_logs`），不覆盖 AI 会话库。 | 实施期澄清，由任务 9 的评审提出。工具结果会原样持久化进 `conversation_entity.Message.Content`/blocks（`conversation.go:72,95`），因此预签名 URL 的签名参数仍以明文存在于会话表里。**不脱敏是有意的**：预签名 URL 正是 `presign` 的交付物，用户要从对话里把它复制走，脱掉这个功能就没有了——D10 本身就要求返回的 URL 完整可用。而且这不是 OSS 独有的性质：任何 SSH 命令打印出的密钥、任何 SQL 查回的口令列，在会话库里都是同样下场，属于会话存储的横切问题，不该在本轮由 OSS 单独开一个先例。因此 issue #250 那条验收标准按「审计 = `audit_logs`」理解；会话库的静态加密/保留策略是另一件事。 |
...
| **D20 更正** | 丢弃条件精确化为「resource 以 `/` 收尾 **且 action 不是列举**（`*.list`）」；来源不参与此判断。 | 实施期更正，由任务 20 的实现者发现并提出。D20 原文把条件写成「resource 以 `/` 结尾」，那是**症状**不是**病因**：它真正要防的是「命令指名一个对象、规则却读成一片前缀」这个落差。该落差只存在于单对象动作 —— `object delete b/logs/` 说的是那个零字节目录标记，规则说的是递归删除，差一个数量级。列举没有这个落差：`object list b/logs/` 与 `object.list b/logs/` 指的是同一个范围。按原措辞一刀切，会与 §4.3【实施期更正】里「`DirList` 的主体可以是前缀形状且应当落库」当场对撞，并且让同一个字符串在 exec 面被丢、在 cp 面被留，破坏 §6.2「两条入口的授权互相复用」。否决「DirList 主体也一并丢弃」：那会让递归 `cp` 的枚举步骤永远拿不到常驻授权、每次重新弹框。 |
...
| **D19** | 条目上限 200，超出报错；快速失败；不跟随符号链接。 | 否决"不设上限"：五千条的审批对话框是橡皮图章，不是决策。否决"静默截断到前 200 条"：一次看起来成功、实际只复制了一部分的传输，比报错糟得多。否决"POSIX 式继续并最终非零"：每个已传输字节都是已批准的副作用，出意外后继续会留下看似完整、实则残缺的目的地。**【实施期更正】**原文还援引了"重跑代价很低（grant 已落库）"——多源 cp 走 `batch` 审批而 `batch` 不支持 `allowAll`，因此不落 grant，重跑要重新批准全部条目（§6.5 的实施期更正）。该援引作废；快速失败仍然成立，理由是中止点已知而继续后的残缺位置未知。否决"跟随符号链接"：一条指向 `/` 的链接能把 `cp -r ./dir` 变成整机 dump，而逃逸出去的路径不在用户审阅过的清单里。 |
