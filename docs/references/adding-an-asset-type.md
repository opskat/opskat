...
| Password credentials and SSH keys | `credential_svc`, `credential_mgr_svc`, `credential_resolver`, `asset_put_svc`, `credential_query_svc` | `credentialConfig.ts`, `useAssetCredential.ts`, `PasswordSourceField.tsx`, and SSH key controls | `credential_id` is not globally one thing. The type-owned automation contract declares accepted kinds; `asset_put_svc` validates existing references and encrypts supported asset-local plaintext, while `credential_query_svc` exposes only typed-ref safe metadata. |
...
Transport selection convention: an explicit `proxy_chain` wins; legacy tunnel/SOCKS fields are normalized only when no new chain is saved. Resolve the chain through `credential_resolver.ResolveProxyChain`, then use `connpool.ProxyChainDialContext` for standard TCP/HTTP transports. `tlsWrappedDialFunc` adds manual TLS wrapping for drivers whose own TLS options are bypassed or rejected when a custom dialer is set (go-redis, franz-go). Chain secrets remain encrypted at rest and are resolved once at the connection boundary.
...
- existing managed-credential reference behavior and write-only asset-local fields (or explicit credential inapplicability); direct-encrypted secrets such as K8s kubeconfig must be distinguished from references;
...
7. Add `internal/ai/skills/<type>/SKILL.md` with the complete `put_asset.config` contract; its field table must match `AutomationContract.ConfigFields` and explain write-only asset-local/reference semantics.
...
On the backend, credential storage and SSH key management live in `credential_svc`, `credential_mgr_svc`, and `credential_resolver`. Automation writes additionally go through `asset_put_svc`: the handler contract declares what is accepted, and the service validates/binds existing references before `Apply*Args`; supported plaintext is encrypted asset-locally by the handler. Do not add an automation path that creates/imports managed credentials. Safe AI/opsctl discovery goes through `credential_query_svc` using `credential:<id>` / `agent-source:<id>` refs; it never reveals secrets or Agent endpoints. K8s remains the exception in shape, not a credential: kubeconfig is directly encrypted asset config. `local` and `serial` have no credentials.
