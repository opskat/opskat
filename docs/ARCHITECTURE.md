...
| `helper` | Protocol clients (SSH, SQL, Redis, Mongo, Kafka, etcd, Serial) that each run the permission check before executing and record the decision. (K8s tool execution lives in `tool/tool_handler_k8s.go`; RDP, VNC, and built-in OSS browsing are interactive app surfaces rather than AI helpers.) |
