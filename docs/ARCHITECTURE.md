...
| `tool` | The tool registry. `Tools()` exposes the business tools to the AI as cago `tool.Tool`s (asset / exec / data / extension groups) **plus** the unified `exec` / `help` pair (`tools_unified.go`); `AllToolDefs()` is the name→handler table opsctl shares and is **not** a mirror of it — it omits `exec` / `help` (desktop-only for now), `run_serial_command`, and `batch_command`. Extensions are reached through the **single** `exec_tool` dispatcher (`tool_handler_ext.go`), not one AI tool per extension. |
...
| `helper` | Protocol clients (SSH, SQL, Redis, Mongo, Kafka, etcd, Serial, K8s). **Two tiers — check before you call:** the `Handle*` entry points (e.g. `HandleExecSQL`, `HandleRunSerialCommand`) run the permission check and record the decision — **except the seven `HandleKafka*`**, which stopped doing so when the `kafka_*` tools were deleted and are now reached only through `ExecKafkaOnAsset`; the `Exec*OnAsset` entry points (`ExecCommandOnAsset` / `ExecSQLOnAsset` / `ExecRedisOnAsset` / `ExecSerialOnAsset` / `ExecK8sOnAsset` / `ExecMongoOnAsset` / `ExecEtcdOnAsset` / `ExecKafkaOnAsset`) are **pure execution bodies with no permission check at all** — they exist so the unified `exec` tool and the legacy per-type tools share one execution path, and the caller is responsible for having checked. Calling one from a new site without a `permission` check bypasses policy. Enumerate them with `git grep -n "^func Exec.*OnAsset" -- internal/ai/helper/`. (RDP, VNC, and built-in OSS browsing are interactive app surfaces rather than AI helpers.) |
...
| `assetref` | Resolves the `asset` tool argument (numeric id **or** name) to an `*asset_entity.Asset`; a name matching more than one asset is an error rather than a silent first-match. Used by `exec` / `help` and by the audit middleware. |
