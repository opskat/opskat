...
`pkg/extension/` is a **wazero** WASM runtime. Each extension ships a `manifest.json` declaring its tools (JSON-Schema params), asset types, capabilities, and policy type; the WASM module is a WASI **reactor** (`GOOS=wasip1 go build -buildmode=c-shared`) exporting `opskat_call` plus `malloc` / `free`, and importing exactly two host functions — `host_call` (JSON control plane) and `host_io` (binary data plane). Key files: `runtime.go` (compile, instance pool, `Plugin.CallTool`), `invocation.go` (per-call IO handle table + cancellation), `manifest.go` (parse + capability checks), `host.go` / `host_module.go` / `hostfn.go` / `host_default.go` (the host capability surface), `tool_reference.go` (renders `tools[].parameters` into the `help` body), `bridge.go` (the name → loaded-extension lifecycle table), `manager.go` (scan/load the extensions dir).

A manifest **must** declare `assetTypes[]` and `policies.type`: extension tools are reached through `exec` on an asset, so an extension without an asset type has no reachable entry point and is refused at load.
...
**Lifecycle:** `internal/service/extension_svc` scans the extensions dir at startup, applies enabled/disabled state from `extension_state_repo`, and registers each enabled extension through `internal/extreg`; KV data is persisted via `extension_data_repo`. Enable / disable / install / uninstall drive the same path and persist state.

`internal/extreg` is the single seam that wires one loaded extension into the app-wide registries the built-in asset types use: an `assettype.AssetTypeHandler` derived from the manifest's `configSchema`, a `permission` executor + `CanonicalizeFunc` + `PolicyCheckFunc` for its asset type, its `SKILL.md` plus a tool/parameter reference rendered from `tools[].parameters` as the `help` document, and its `ext:` policy groups. Registration is all-or-nothing and **refuses an asset type another extension or a built-in type already owns** — asset type → extension is one-to-one because `exec` dispatches through it. Unregistering removes every one of those entries. The frontend mirrors this: `frontend/src/extension/assetTypes.ts` turns the same manifest into an `AssetTypeDefinition` in the shared asset-type registry, so no UI surface branches on "is this an extension type".
