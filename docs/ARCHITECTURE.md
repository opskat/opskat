...
| `tool` | The tool registry. `Tools()` exposes **15** tools to the AI as cago `tool.Tool`s: 13 business tools (asset / exec / extension groups) plus the unified `exec` / `help` pair (`tools_unified.go`), which is the **only** way to run a command on any asset type — the 14 per-type tools (`run_command`, `exec_sql`, `kafka_*`, …) were deleted once `exec` covered them. `AllToolDefs()` is the name→handler table opsctl shares and is **not** a mirror of it — it omits only `batch_command` (opsctl has its own `batch` subcommand). Extensions are reached through the **single** `exec_tool` dispatcher (`tool_handler_ext.go`), not one AI tool per extension. |
...
| `helper` | Protocol clients (SSH, SQL, Redis, Mongo, Kafka, etcd, Serial, K8s). **No entry point here checks permissions.** The `Exec*OnAsset` functions (`ExecCommandOnAsset` / `ExecSQLOnAsset` / `ExecRedisOnAsset` / `ExecSerialOnAsset` / `ExecK8sOnAsset` / `ExecMongoOnAsset` / `ExecEtcdOnAsset` / `ExecKafkaOnAsset`) are **pure execution bodies**; the check happens exactly once, in `tool.handleExec`, before dispatch. The `Handle*` variants that used to check were deleted along with the per-type tools that called them. Calling an `Exec*OnAsset` from a new site without a `permission` check bypasses policy. Enumerate them with `git grep -n "^func Exec.*OnAsset" -- internal/ai/helper/`. (RDP, VNC, and built-in OSS browsing are interactive app surfaces rather than AI helpers.) |
...
| `assetref` | Resolves the `asset` tool argument (numeric id **or** name) to an `*asset_entity.Asset`; a name matching more than one asset is an error rather than a silent first-match. Used by `exec` / `help` / `batch_command` and by the audit middleware. |
...
| `execimpl` | Wires `helper`'s `Exec*OnAsset` bodies into `permission`'s executor registry in `init()`. **Invisible by design:** nothing calls it — `tool` reaches it through a blank import, which is the only trigger for the registration. It must **not** import `internal/ai/tool` (that would close an import cycle), so the registration direction is bottom-up: `execimpl` pushes executors into `permission`. Giving a new asset type `exec` support means adding a `skills` entry and a `RegisterExecutor` call here. |
...
**The unified `exec` tool's ordering is load-bearing** (`tool/tool_handlers_unified.go`): resolve the asset → look up its executor (unregistered type → explicit error) → **doc gate** (type's syntax doc not yet seen this conversation → return guidance text, not a Go error, so the model can call `help` and retry in the same turn) → canonicalize the command for types that register a `CanonicalizeFunc` (only k8s, which injects `--context`/`--namespace`) → per-type `PrecheckFunc` (only serial, which requires a connected session) → **then** the permission check. Every side-effect-free judgement must run *before* `CheckForAsset`, because `NeedConfirm` blocks on a user-visible approval dialog whose "allow all" persists a grant — popping it for a call that was already doomed broadens permissions through a prompt the user should never have seen. The check runs against the *canonicalized* command (what policy, the approval dialog, and the audit row all show) while execution receives the *raw* one. The three short-circuit paths record a `decision=deny` with an `exec_*` `DecisionSource` so the audit log can tell them apart from a command that actually ran. The check itself is **fail-closed**: `permission.getPolicyChecker` is unexported, so the only ways to reach a checker are `RequireChecker` (errors when absent) and `RequireCheckerOrPreapproved` (returns no checker only when the caller marked the context with `WithPreapproved` — opsctl, which already ran `requireApproval` before dispatching). A missing injection can no longer read as "allow".
