...
Asset automation has two shared service boundaries. `internal/service/asset_put_svc` consumes the selected `assettype.AutomationContract`: it rejects undeclared config fields, applies type-owned normalization/credential planning, validates referenced credential or SSH Agent associations, and exposes only safe approval/audit views. `Prepare` is side-effect-free; `Commit` materializes a plaintext password/OSS secret as a managed password credential or imports an SSH private key, binds it through the type owner, and writes credential + asset inside one `dbutil` transaction. AI `put_asset` calls this same boundary directly. `internal/service/credential_query_svc` is the shared read boundary for AI and opsctl: typed refs (`credential:<id>` / `agent-source:<id>`) produce password/SSH-key/Agent metadata, usage and bounded runtime status without secret or Agent endpoint disclosure; `get_asset` asks the registered type owner for an authentication association and enriches only that one detail response.

End to end — `opsctl create asset`: parse generic `--config` or `--config-file` and explicitly supplied compatibility flags → resolve files, SSH asset references and credential/Agent inputs → call `asset_put_svc.Prepare` so unknown fields, defaults, auth conflicts and references are checked before approval → send the safe detail over `approval.sock` → on approval call `asset_put_svc.Commit` for the single credential+asset transaction → notify the desktop, write the producer-owned Audit projection (`SafeAuditArgsForResult` field allowlist): ordinary config and identity plus the typed credential reference are retained, write-only secret fields are absent — no generic redactor — and print the safe ID/authentication result. Denial or any prepare/commit failure leaves no newly created credential or asset row.
