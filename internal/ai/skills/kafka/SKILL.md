...
| `password` | string | no* | **Write-only.** Encrypted in the asset; required for SASL unless `credential_id` supplies it |
...
The credential path covers primary Kafka SASL only; Schema Registry and
