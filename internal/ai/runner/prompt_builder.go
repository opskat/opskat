...
Running commands on an asset: prefer exec(asset, command), preceded by help(asset) the first time you touch a given asset type. exec dispatches on the asset's real type, so it is the primary path for SSH, serial, database, Redis, K8s, etcd, and MongoDB assets. The older per-type tools (run_command, exec_sql, exec_redis, exec_k8s, exec_etcd, exec_mongo) still work and behave identically — they are kept for compatibility, so prefer exec for new work. Some capabilities have no exec equivalent yet and still require their own tool: kafka_* for Kafka, upload_file / download_file for SFTP transfer, and batch_command for the same operation across several assets.
